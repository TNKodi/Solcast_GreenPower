...
class ForecastService:
    ...
    async def process_single_asset(
        ...
        try:
            ...
            if not is_valid:
                ...
                return False, "missing_required_attributes", missing, None
            ...
            if telemetry_records and write_telemetry:
                try:
                    ...
                    return False, f"telemetry_write_failed: {str(e)}", None, None
            else:
    ...
    def _telemetry_list_to_map(
        ...
        for row in telemetry_list:
            ts = int(row.get("ts", 0))
            value = float(row.get("values", {}).get("solcast_daily", 0.0))
    ...
    def _telemetry_map_to_list(self, telemetry_map: Dict[int, float]) -> List[Dict[str, Any]]:
        return [
            {
                "ts": ts,
                "values": {"solcast_daily": float(value)}
            }
    ...
    def _telemetry_total_kwh(self, telemetry_list: List[Dict[str, Any]]) -> float:
        return float(
            sum(
                float(row.get("values", {}).get("solcast_daily", 0.0))
                for row in telemetry_list
            )
        )
